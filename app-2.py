...
import uuid, qrcode, io, base64, os, urllib.parse, time
...
def safe_select(table_name, retries=3, delay=1):
    """يعيد محاولة الاتصال بـ Supabase عند فشل الشبكة المؤقت (مثل سكون السيرفر المجاني)"""
    last_err = None
    for attempt in range(retries):
        try:
            ...
            last_err = e
            time.sleep(delay)
    print(f"⚠️ فشل الاتصال بجدول {table_name} بعد {retries} محاولات: {last_err}")
    return []
...
def get_settings():
    try:
        rows = safe_select("store_settings")
...
def admin_dashboard():
    try:
        products     = safe_select("products")
        transactions = safe_select("transactions")
        orders       = safe_select("orders")
...
@admin_required
def admin_orders():
    orders = safe_select("orders") or []
    products_all = safe_select("products") or []
...
@admin_required
def admin_sales():
    transactions = safe_select("transactions") or []
    products     = safe_select("products") or []
    s = get_settings()
...
@admin_required
def admin_finance():
    wallets   = safe_select("wallets") or []
    capitals  = safe_select("capital_accounts") or []
    logs      = safe_select("financial_logs") or []
    users     = load_users()
    s = get_settings()
...
def admin_settings():
    ...
    users = load_users()
    coupons = []
    try: coupons = safe_select("coupons") or []
    except: pass
    products_all = safe_select("products") or []
...
def admin_reports():
    ...
    products     = safe_select("products") or []
    reviews      = []
    try: reviews = safe_select("reviews") or []
    except: pass
    s = get_settings()
...
def admin_search():
    ...
    if query:
        if section=="المنتجات":
            results=[p for p in safe_select("products") if query.lower() in p["name"].lower()]
        elif section=="المبيعات":
            results=[t for t in safe_select("transactions") if query.lower() in t["product_name"].lower() or query.lower() in t.get("seller","").lower()]
        elif section=="الطلبات":
            results=[o for o in safe_select("orders") if query.lower() in o.get("customer_name","").lower() or query in o.get("phone","") or query.lower() in o.get("city","").lower()]
        elif section=="السجل المالي":
            results=[l for l in safe_select("financial_logs") if query.lower() in l.get("action_type","").lower() or query.lower() in l.get("note","").lower()]
    s = get_settings()
